...
function translateHtml(html, translations, locale) {
  ...
  html = html.replace(
    ...
    (match, before, key, mid, placeholder, after) => {
      const translated = translations[key];
      if (translated !== undefined) {
        return before + `data-i18n-placeholder="${key}"` + mid + translated + after;
      }
      return match;
    }
  );
  ...
  html = html.replace(
    ...
    (match, before, key, mid, label, after) => {
      const translated = translations[key];
      if (translated !== undefined) {
        return before + `data-i18n-aria-label="${key}"` + mid + translated + after;
      }
      return match;
    }
  );
 ...
 */
function addHreflangLinks(html, locales, currentLocale) {
    ...
    closeBundle() {
      ...
      for (const locale of locales) {
        ...
        for (const inputFile of inputFiles) {
          ...
          // Add hreflang links
          html = addHreflangLinks(html, locales, locale);
      ...
      for (const inputFile of inputFiles) {
        ...
        let html = readFileSync(sourcePath, 'utf-8');
        html = addHreflangLinks(html, locales, defaultLocale);
